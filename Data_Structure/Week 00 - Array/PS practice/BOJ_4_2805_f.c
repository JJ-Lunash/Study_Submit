...
int ft_binsearch(int start, int end, int *tree_arr, int m)
{
	int mid = (start + end) / 2;
	int i = 0;
...
int main(){
    ...
    long long mid = 0, max = 0; // start,end,mid: 배열 인덱스, max : 가장 높은 나무의 길이
    int i = 0;
    int h = 0; // h: 절단기 높이
    int tree_arr[1000000] = {0}; // 각 나무의 높이 저장한 배열
    ...
    printf("%d", ft_binsearch(start, end, n, m));
